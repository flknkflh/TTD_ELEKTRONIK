...
curl -sS -X POST "$BASE/api/v1/auth/register" -H 'Content-Type: application/json' \
  -d '{"email":"admin@local","password":"admin12345","role":"admin","full_name":"Administrator","organization":"Lab"}' >/dev/null || true
...
UID_=$(curl -sS -X POST "$BASE/api/v1/auth/register" -H 'Content-Type: application/json' \
  -d '{"email":"user@local","password":"user12345","role":"user","full_name":"Budi Santoso","organization":"Dinas Kominfo"}' | jval account_id || true)
