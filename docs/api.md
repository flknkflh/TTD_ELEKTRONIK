...
✔ GET    /api/v1/admin/ca/issuer                     (super admin only: {mode: offline|lab|online, state: pending|active, intermediate?})
✔ GET    /api/v1/admin/ca/intermediate.csr           (super admin only: the online issuer's pending Intermediate CSR)
✔ POST   /api/v1/admin/ca/intermediate               (super admin only: install the Root-signed Intermediate certificate; during a rotation it replaces the active one and retires it; a CRL follows)
✔ POST   /api/v1/admin/ca/rotate                     (super admin only: prepare the next Intermediate key + CSR now; also automatic PQC_CA_ROTATE_DAYS before expiry)
